...
class TranslationService {
    ...
    async detectLanguage(text) {
        ...
        try {
            const response = await this.deepSeek.generateResponse([{
                role: 'user',
                content: `Определи язык этого текста и верни только код языка (en, ru, etc.): "${text.substring(0, 100)}"`
            }]);
            ...
            console.error('Error detecting language, using fallback:', error);
            return this.fallbackDetectLanguage(text);
        }
    }

    fallbackDetectLanguage(text) {
        if (text.length < 3) return 'en';
    ...
    async translateText(text, targetLanguage, sourceLanguage = 'auto') {
        ...
        if (text.length > 4000) return text; // Слишком длинный текст
        
        try {
            const response = await this.deepSeek.generateResponse([{
                role: 'user',
                content: `Переведи этот текст с ${sourceLanguage} на ${targetLanguage}: "${text.substring(0, 1000)}"`
            }]);
